...
void ULockOnComponent::TargetLockOff()
{
	SetupLocalPlayerController();
...
TArray<AActor*> ULockOnComponent::GetAllActorsOfClass(TSubclassOf<AActor> ActorClass) const
{
...
TArray<AActor*> ULockOnComponent::FindTargetsInRange(TArray<AActor*> ActorsToLook, float RangeMin, float RangeMax) const
{
...
AActor* ULockOnComponent::FindNearestTarget(TArray<AActor*> Actors) const
{
...
bool ULockOnComponent::LineTrace(FHitResult& OutHitResult, const AActor* OtherActor, const TArray<AActor*>& ActorsToIgnore) const
{
...
bool ULockOnComponent::LineTraceForActor(const AActor* OtherActor, const TArray<AActor*>& ActorsToIgnore) const
{
...
bool ULockOnComponent::ShouldBreakLineOfSight() const
{
	if (!LockedOnTargetActor)
		return true;
...
bool ULockOnComponent::IsInViewport(const AActor* TargetActor) const
{
	if (!IsValid(OwnerPlayerController))
		return true;
	...
	if (ShouldBreakLineOfSight() && !bIsBreakingLineOfSight)
	{
		...
		else
		{
			...
			GetWorld()->GetTimerManager().SetTimer(LineOfSightBreakTimerHandle, this,
				&ULockOnComponent::BreakLineOfSight, BreakLineOfSightDelay);
		}
	}
}

void ULockOnComponent::TargetActor()
{
...
void ULockOnComponent::TargetActorWithAxisInput(float AxisValue)
{
	...
	if (ActorToTarget)
	{
		...
		GetWorld()->GetTimerManager().SetTimer(SwitchingTargetTimerHandle, this,
			&ULockOnComponent::ResetIsSwitchingTarget, bIsSwitchingTarget ? 0.25f : 0.5f);

		bIsSwitchingTarget = true;
	}
}

FRotator ULockOnComponent::GetControlRotationOnTarget(const AActor* OtherActor) const
{
	if (!IsValid(OwnerPlayerController))
...
void ULockOnComponent::SetControlRotationOnTarget(AActor* TargetActor) const
{
	if (!IsValid(OwnerPlayerController))
		return;
...
void ULockOnComponent::ControlRotation(bool ShouldControlRotation) const
{
...
float ULockOnComponent::GetAngleUsingCameraRotation(const AActor* ActorToLook) const
{
...
float ULockOnComponent::GetAngleUsingCharacterRotation(const AActor* ActorToLook) const
{
...
void ULockOnComponent::CreateAndAttachTargetLockedOnWidgetComponent(AActor* TargetActor)
{
...
void ULockOnComponent::TargetLockOn(AActor* TargetToLockOn)
{
...
void ULockOnComponent::ResetIsSwitchingTarget()
{
	bIsSwitchingTarget = false;
	bDesireToSwitch = false;
}

bool ULockOnComponent::ShouldSwitchTargetActor(float AxisValue)
{
...
void ULockOnComponent::SetupLocalPlayerController()
{
	if (!IsValid(OwnerPawn))
		return;
